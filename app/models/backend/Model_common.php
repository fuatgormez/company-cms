...
class Model_common extends CI_Model 
{
    ...
    public function add_column($table_name, $field, $type = 'VARCHAR', $constraint = 100){
        ...
        return $this->dbforge->add_column($table_name, $field);  
    }

    public function drop_column($table_name, $field)
    {
        return $this->dbforge->drop_column($table_name, $field);  
    }
    ...
    public function extension_check_photo($ext) {
    	if( $ext!='svg' && $ext!='SVG' && $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' && $ext!='JPG' && $ext!='PNG' && $ext!='JPEG' && $ext!='GIF' ) {
    		return false;
    	} else {
    		return true;
    	}
