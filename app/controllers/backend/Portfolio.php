...
class Portfolio extends CI_Controller 
{
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
	            ...
	            if( isset($_FILES['photos']["name"]) && isset($_FILES['photos']["tmp_name"]) )
		        {
		            ...
		            for($i=0;$i<count($photos);$i++)
		            {
				        ...
				        if($ext_check == FALSE) {
				        	...
				        	$final_names[$m] = $z.'.'.$ext;
		                    move_uploaded_file($photos_temp[$i],"./public/uploads/portfolio/".$final_names[$m]);
		                    $m++;
		                    $z++;
				        }
		            }
		    ...
		    else
		    {
				...
				redirect(base_url(BACKEND.'portfolio/add'));
		    }            
        } else {
            $data['all_photo_category'] = $this->Model_portfolio->get_all_photo_category();
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'portfolio/portfolio_add',$data);
			$this->load->view(BACKEND.'footer');
        }
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    if($valid == 1) 
		    {
		    	...
		    	if($path == '') {
					...
					unlink('./public/uploads/portfolio/'.$data['portfolio']['photo']);

					$final_name = 'portfolio-'.$id.'.'.$ext;
		        	move_uploaded_file( $path_tmp, './public/uploads/portfolio/'.$final_name );
		    ...
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url(BACKEND.'portfolio/edit/'.$id));
		    }
			...
			$data['all_photos_by_id'] = $this->Model_portfolio->get_all_photos_by_category_id($id);
	       	$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'portfolio/portfolio_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
	...
	public function delete($id) 
	{
        ...
        $portfolio = $this->Model_portfolio->get_all_photos_by_category_id($id);
        foreach($portfolio as $row) {
			unlink('./public/uploads/portfolio/'.$row['photo']);
        }
    ...
    public function single_photo_delete($photo_id=0,$portfolio_id=0) {
  		...
  		unlink('./public/uploads/portfolio/'.$portfolio_photo['photo']);

  		$this->Model_portfolio->delete_portfolio_photo($photo_id);

  		redirect(base_url(BACKEND.'portfolio/edit/'.$portfolio_id));
    }
