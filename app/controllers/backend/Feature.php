...
class Feature extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model(BACKEND.'Model_feature');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'feature/feature',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
				...
				redirect(base_url(BACKEND.'feature'));
		    } 
		    else
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url(BACKEND.'feature/add'));
		    }
            
        } else {
            
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'feature/feature_add',$data);
			$this->load->view(BACKEND.'footer');
        }
	...
	public function edit($id)
	{
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url(BACKEND.'feature/edit/'.$id));
		    }
           
		} else {
			$data['feature'] = $this->Model_feature->getData($id);
	       	$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'feature/feature_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
