...
class Partner extends CI_Controller 
{
	...
	{
        parent::__construct();
        $this->load->model(BACKEND.'Model_header');
        $this->load->model(BACKEND.'Model_partner');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'partner/partner',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
		        ...
		        move_uploaded_file( $path_tmp, './public/uploads/partner/'.$final_name );
            ...
            $this->load->view(BACKEND.'header',$header);
			$this->load->view(BACKEND.'partner/partner_add',$data);
			$this->load->view(BACKEND.'footer');
	...
	public function edit($id)
	{
    	...
    	if(!$tot) {
    		redirect(base_url(BACKEND.'partner'));
        	exit;
    	}
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    $data['partner'] = $this->Model_partner->getData($id);
	       	$this->load->view(BACKEND.'header',$header);
			$this->load->view(BACKEND.'partner/partner_edit',$data);
			$this->load->view(BACKEND.'footer');
	...
	public function delete($id) 
	{
        ...
        $data['partner'] = $this->Model_partner->getData($id);
        if($data['partner']) {
            unlink('./public/uploads/partner/'.$data['partner']['photo']);
        }
