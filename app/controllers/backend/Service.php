...
class Service extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model(BACKEND.'Model_service');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'service/service',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
		        ...
		        move_uploaded_file( $path_tmp, './public/uploads/service/'.$final_name );
				...
				redirect(base_url('backend/service'));
		    } 
		    else
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url('backend/service/add'));
		    }
            
        } else {
            
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'service/service_add',$data);
			$this->load->view(BACKEND.'footer');
        }
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    if($valid == 1) 
		    {
		    	...
		    	if($path == '') {
					...
					unlink('./public/uploads/service/'.$data['service']['photo']);
		    ...
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url('backend/service/edit/'.$id));
		    }
           
		} else {
			$data['service'] = $this->Model_service->getData($id);
	       	$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'service/service_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
