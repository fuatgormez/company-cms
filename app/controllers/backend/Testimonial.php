...
class Testimonial extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model(BACKEND.'Model_testimonial');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'testimonial/testimonial',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
		        ...
		        move_uploaded_file( $path_tmp, './public/uploads/testimonial/'.$final_name );
            ...
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'testimonial/testimonial_add',$data);
			$this->load->view(BACKEND.'footer');
        }
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    if($valid == 1) 
		    {
		    	...
		    	if($path == '') {
					...
					unlink('./public/uploads/testimonial/'.$data['testimonial']['photo']);
		    	...
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url(BACKEND.'testimonial/edit'.$id));
		    }
           
		} else {
			$data['testimonial'] = $this->Model_testimonial->get_testimonial($id);
	       	$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'testimonial/testimonial_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
