...
class Setting extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'setting/setting', $data);
	...
	public function update()
	{
		...
		if (isset($_POST['form_loader'])) {
			$valid = 1;
			$path = $_FILES['photo_loader']['name'];
			$path_tmp = $_FILES['photo_loader']['tmp_name'];
			if ($path != '') {
				$ext = pathinfo($path, PATHINFO_EXTENSION);
				$file_name = basename($path, '.' . $ext);
				$ext_check = $this->Model_common->extension_check_photo($ext);
			...
			if ($valid == 1) {
				// removing the existing photo
				unlink('./public/uploads/' . $data['setting']['loader']);
				...
				$this->session->set_flashdata('success', $success);
				redirect(base_url(BACKEND.'setting#loader'));
			} else {
				$this->session->set_flashdata('error', $error);
				redirect(base_url(BACKEND.'setting#loader'));
			}
	...
	public function cache() {
		...
		$this->session->set_flashdata('success', 'Cache has been removed successfully!');
		redirect(base_url(BACKEND.'setting'));
	}

	public function log() {
		$this->load->library('logs');
		$this->logs->clear_all_log();

		$this->session->set_flashdata('success', 'Logs has been removed successfully!');
		redirect(base_url(BACKEND.'setting'));
	}
}
