...
class Portfolio_category extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model(BACKEND.'Model_portfolio_category');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'portfolio/portfolio_category',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
				...
				redirect(base_url(BACKEND.'portfolio_category'));
		    }
		    else
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url(BACKEND.'portfolio_category/add'));
		    }
            
        } else {
            
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'portfolio/portfolio_category_add',$data);
			$this->load->view(BACKEND.'footer');
        }
	...
	public function edit($id)
	{
		...
		if(isset($_POST['form1'])) 
		{
			...
			$data['portfolio_category'] = $this->Model_portfolio_category->getData($id);
			$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'portfolio/portfolio_category_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
	...
	public function delete($id) 
	{
		...
		foreach ($result as $row) {
			...
			foreach ($result1 as $row1) {
				$photo = $row1['photo'];
				unlink('./public/uploads/portfolio/'.$photo);
			}
