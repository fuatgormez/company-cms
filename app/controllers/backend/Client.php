...
class Client extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model('backend/Model_client');
    }
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
				...
				redirect(base_url('backend/client'));
		    } 
		    else
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url('backend/client/add'));
		    }
            
        } else {
            
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'client/client_add',$data);
			$this->load->view(BACKEND.'footer');
        }
	...
	public function edit($id)
	{
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url('backend/client/edit'.$id));
		    }
           
		} else {
			$data['client'] = $this->Model_client->get_client($id);
	       	$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'client/client_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
