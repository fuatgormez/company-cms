...
class Event extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model(BACKEND.'Model_event');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'event/event',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
		        ...
		        move_uploaded_file( $path_tmp, './public/uploads/event/'.$final_name );

		        $final_name1 = 'event-banner-'.$ai_id.'.'.$ext1;
		        move_uploaded_file( $path_tmp1, './public/uploads/event/'.$final_name1 );
		    ...
		    else
		    {
				...
				redirect(base_url(BACKEND.'event/add'));
		    }
            
        } else {
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'event/event_add',$data);
			$this->load->view(BACKEND.'footer');
        }
	...
	public function edit($id)
	{
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    if($valid == 1) 
		    {
				...
				if($path != '' && $path1 == '') {
					unlink('./public/uploads/event/'.$data['event']['photo']);
				...
				if($path != '' && $path1 != '') {
					...
					unlink('./public/uploads/event/'.$data['event']['photo']);
					unlink('./public/uploads/event/'.$data['event']['banner']);
			...
			$data['event'] = $this->Model_event->getData($id);
            $this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'event/event_edit',$data);
			$this->load->view(BACKEND.'footer');
		}
