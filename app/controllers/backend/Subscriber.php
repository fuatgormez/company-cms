...
class Subscriber extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model(BACKEND.'Model_subscriber');
    }
	...
	public function index()
	{
		...
		$this->load->view(BACKEND.'subscriber/subscriber',$data);
	...
	public function delete($id)
	{
    	...
    	if(!$tot) {
    		redirect(base_url(BACKEND.'subscriber'));
    	}
    ...
    public function send_email()
	{
		...
		if(isset($_POST['form1'])) {
            ...
            if($valid == 1) 
            {
				...
				$this->session->set_flashdata('success',$success);
				redirect(base_url(BACKEND.'subscriber/send_email'));
            }
            else
            {
		...
		{
			$this->load->view(BACKEND.'header',$data);
			$this->load->view(BACKEND.'subscriber/subscriber_email',$data);
			$this->load->view(BACKEND.'footer');
		}
	}
