...
class Newsletter extends CI_Controller {
	function __construct()
	{
        ...
        $this->load->model(FRONTEND.'Model_newsletter');
    }
	...
	public function send() {
		...
		if(isset($_POST['form_subscribe'])) {
		    ...
		    {
		    	if (filter_var($this->input->post('email_subscribe'), FILTER_VALIDATE_EMAIL) === false)
			    {
			        $valid = 0;
			    ...
			    {
			    	$total = $this->Model_newsletter->total_subscriber_by_email($this->input->post('email_subscribe'));
	            	if($total) {
	            		$valid = 0;
		    ...
		    if($valid == 1)
		    {
		    	...
		    	$form_data = array(
					'subs_email' => $this->input->post('email_subscribe'),
	            ...
	            $verification_url = base_url('newsletter/verify/'.$this->input->post('email_subscribe').'/'.$key);
	...
	function verify($email=0,$hash=0) {
		...
		$this->load->view(FRONTEND.'thankyou_subscribe',$data);
